...
if (!(Get-Command curl.exe -ErrorAction SilentlyContinue)) {
    Write-Host "[0/6] Installing curl..."
...
if (!(Test-Path ".env")) {
    if (Test-Path ".env.example") {
        ...
        Write-Host "[1/6] Created .env from .env.example - please fill in your values."
    } else {
        Write-Host "[1/6] WARNING: No .env file found. Create one before running."
        exit 1
    }
} else {
    Write-Host "[1/6] .env file exists."
}

# -- Step 3: Start infrastructure services only if not already running
Write-Host "[2/6] Ensuring infrastructure services are running..."
docker compose up -d ollama chromadb redis

# -- Step 4: Rebuild and redeploy application services (picks up code changes)
Write-Host "[3/6] Rebuilding and redeploying application services..."
docker compose up -d --build --force-recreate api mcp

# -- Step 5: Rebuild and redeploy UI
Write-Host "[4/6] Rebuilding and redeploying UI..."
docker compose up -d --build --force-recreate ui

# -- Step 6: Read model names from .env and pull if missing
Write-Host "[5/6] Checking Ollama models..."
...
try {
    $apiResp = curl.exe -sf http://localhost:8081/health 2>$null
    Write-Host "  API (http://localhost:8081/health): [UP] $apiResp"
} catch {
    Write-Host "  API (http://localhost:8081/health): [DOWN]"
}

try {
    curl.exe -sf http://localhost:8501/_stcore/health 2>$null | Out-Null
    Write-Host "  UI (http://localhost:8501/_stcore/health): [UP]"
} catch {
    Write-Host "  UI (http://localhost:8501/_stcore/health): [DOWN]"
}
...
Write-Host "  MCP:      http://localhost:8082/mcp"